...
CREATE TABLE IF NOT EXISTS `tfg`.`clientes` (
  ...
  INDEX `fk_clientes_usuarios1_idx` (`usuarios_nombre` ASC),
...
CREATE TABLE IF NOT EXISTS `tfg`.`turnos` (
  ...
  `usuarios_nombre_iniciado_por` VARCHAR(64) NOT NULL,
  PRIMARY KEY (`id`),
  ...
  INDEX `fk_turnos_usuarios1_idx` (`usuarios_nombre_iniciado_por` ASC),
  ...
  CONSTRAINT `fk_turnos_usuarios1`
    FOREIGN KEY (`usuarios_nombre_iniciado_por`)
    REFERENCES `tfg`.`usuarios` (`nombre`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `tfg`.`turnos_has_productos` (
  ...
  PRIMARY KEY (`turnos_id`, `productos_id`),
